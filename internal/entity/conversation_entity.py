...
# 摘要汇总模板
# 中期记忆 工作原理
# 通过递归传递总结的方式，每次只处理"新对话"，然后与"旧总结"合并，形成新的总结。
...
# 当使用 with_structured_output() 时，LangChain 会将整个 ConversationInfo 类的信息传递给 LLM，包括：

# 类的 docstring（所有描述和示例）

# 每个 Field 的 description
# 注释也会被传递过去
# Pydantic 模型的结构信息
class ConversationInfo(BaseModel):
    ...
    #   "description": "你需要将用户的输入分解为\"主题\"和\"意图\"，以便准确识别用户输入的类型...",
    #   "parameters": {
    #     "type": "object",
    #     "properties": {
    #       "language_type": {
    #         "type": "string",
    #         "description": "用户输入语言的语言类型声明"
    #       },
    #       "reasoning": {
    #         "type": "string", 
    ...
    #       "subject": {
    #         "type": "string",
    #         "description": "对用户的输入进行简短的总结，提取输入的\"意图\"和\"主题\"，输出语言必须和输入语言保持一致，尽可能简单明了..."
    #       }
    #     },
    #     "required": ["language_type", "reasoning", "subject"]
    #   }
    # }
