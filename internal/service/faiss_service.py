...
# 是已经生成好的本地 FAISS 索引。这个索引看起来对应的是 storage/vector_store/01.项目API文档.md 这类项目内置 API 文档。原因是辅助 Agent 需要回答“怎么创建应用、接口怎么用、项目 API 是什么”这类平台内置知识，不能依赖用户自己创建的知识库，所以单独加了一个本地 FAISS 服务。
@inject
class FaissService:
    """Faiss向量数据库服务"""
    faiss: FAISS
