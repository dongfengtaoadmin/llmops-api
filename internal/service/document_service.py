...
from internal.model import Account, Dataset, Document, Segment, UploadFile, ProcessRule
...
class DocumentService(BaseService):
    ...
    def create_documents(
            ...
            process_type: str = ProcessType.AUTOMATIC, # 处理类型
            rule: dict = None,
        ...
        for upload_file in upload_files:
            ...
            document = self.create(
                Document,
                account_id=account.id,           # 账号ID，关联创建文档的用户
                dataset_id=dataset_id,           # 知识库ID，文档所属的知识库
                upload_file_id=upload_file.id,   # 上传文件ID，关联原始上传文件记录
                process_rule_id=process_rule.id, # 处理规则ID，关联文档的处理规则（分块、预处理等）
                batch=batch,                     # 批次号，用于批量追踪文档处理进度
                name=upload_file.name,           # 文档名称，继承自上传文件的原始名称
                position=position,               # 文档位置，控制文档在知识库中的显示顺序
            )
