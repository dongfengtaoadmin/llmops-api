...
class CreateDocumentsReq(FlaskForm):
    ...
    # 起名规则
    # 在 Flask-WTF 中，任何以 validate_ 开头、后面跟字段名的方法都会被自动调用。具体到这个类：

    # validate_upload_file_ids 会自动在表单验证时调用，用于校验 upload_file_ids 字段

    # validate_rule 会自动在表单验证时调用，用于校验 rule 字段
