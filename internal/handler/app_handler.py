...
from internal.schema.app_schema import CompletionReq, GenerateConversationNameReq, GenerateSuggestedQuestionsReq
...
class AppHandler:
    ...
    def debug(self, app_id: UUID):
        ...
        def graph_app() -> None:
            ...
            # ✅ 第1次：chatbot 决定需要调用工具

            # ✅ 然后：tool_executor 执行工具

            # ✅ 再然后：回到 chatbot 处理工具结果并生成最终回答

            # 🔄 可循环：如果还需要更多工具，可以多次循环

            # 🛑 最终：当不需要工具时，route() 返回 END 结束

            # 3.5 创建状态图
            graph_builder = StateGraph(MessagesState)

            # 3.6 添加节点
            graph_builder.add_node("llm", chatbot)           # 聊天机器人节点
            graph_builder.add_node("tool_executor", tool_executor)  # 工具执行节点

            # 3.7 添加边
            graph_builder.set_entry_point("llm")             # 入口：llm节点
            graph_builder.add_conditional_edges("llm", route)  # llm后根据条件路由
            graph_builder.add_edge("tool_executor", "llm")   # tool_executor后回到llm
    ...
    def ping(self):
        """测试会话总结功能"""
        # human_message = "你能简单介绍下什么是LLM与Agent么？"
        # ai_message = "当然可以！LLM（Large Language Model，大语言模型）是一种基于深度学习的人工智能模型，通过海量文本数据训练，能够理解和生成人类语言。GPT-4、Claude、文心一言等都是典型的LLM。\n\nAgent（智能体）则是基于LLM构建的能够自主执行任务的系统，它不仅能对话，还能调用工具、做出决策、执行操作。简单来说，LLM是Agent的\"大脑\"，负责思考和理解；Agent是LLM的\"身体\"，负责行动和执行。比如，一个旅行Agent可以使用LLM理解用户需求，然后调用天气API、航班查询工具、酒店预订系统来完成完整的旅行规划任务。"

        # # 调用会话服务生成总结
        # summary = self.conversation_service.summary(
        #     human_message=human_message, 
        #     ai_message=ai_message, 
        #     old_summary=""  # 初始为空，表示第一轮对话
        # )
        ...
        # })
          ...
          # 2. 调用会话服务生成建议问题
        # questions = self.conversation_service.generate_suggested_questions(query)
