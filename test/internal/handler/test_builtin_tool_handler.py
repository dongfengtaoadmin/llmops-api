...
class TestBuiltinToolHandler:
    ...
    # pytest 会自动将测试拆分成2个独立的测试：
    @pytest.mark.parametrize(
        "provider_name, tool_name",  # 参数名称（逗号分隔）
        [                           # 参数值列表
            ("google", "google_serper"),   # 第1组：provider_name="google", tool_name="google_serper"
            ("imooc", "imooc_llmops"),     # 第2组：provider_name="imooc", tool_name="imooc_llmops"
        ]
    )
