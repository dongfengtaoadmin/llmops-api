...
# 拷贝第三方依赖包+源码文件
# COPY --from=<阶段名|阶段索引> <源路径> <目标路径>  
# 将base阶段中的/pkg目录拷贝到/usr/local目录下 防止跟启动目录的 /pkg 冲突
...
# 运行脚本并启动项目 作用： 使用这个镜像创建并启动容器时，默认执行 /entrypoint.sh 脚本。 等价于 /bin/bash /entrypoint.sh
